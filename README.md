...
Code Pen - удобный редактор с четырьмя функциональными окнами: JavaScript, HTML, CSS и предпросмотр результата. Благодаря широким возможностям, при работе над кодом можно подключать различные библиотеки.
JS Bin - помимо блоков для работы с HTML/CSS/JS реализован также вывод в консоль, что очень удобно.
JSFiddle - подходит для командной работы в режиме real time: для этого нужно просто отправить ссылку на проект другому разработчику.
Repl.it - не только песочница, а целая среда для совместной работы с кодом в браузере. Помимо JavaScript, поддерживает более 50 языков программирования.
CodeSandbox - веб-приложение интегрировано с GitHub, поэтому вы можете импортировать файлы из нужного репозитория и работать с ними в песочнице.
